...
5. **Intel iGPU 하드웨어 가속(Vulkan)**: Proxmox LXC 환경에서 인텔 내장 그래픽 가속을 사용하려면 컨테이너 내부의 `ollama` 서비스 사용자 계정에 장치 노드(`/dev/dri/renderD128`) 접근 권한이 확보되어야 합니다. 연산 가속이 적용되지 않는 경우 컨테이너 터미널에서 다음 명령어를 실행하고 서비스를 재시작하십시오:
   ```bash
   chmod 666 /dev/dri/card0 /dev/dri/renderD128
   usermod -aG video,clock ollama
