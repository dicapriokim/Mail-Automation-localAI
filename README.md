...
- **⚡ 하이브리드 배치 요약 엔진 및 병렬화 (V5 Optimization)**: 
  ...
  - **Ollama CPU(N150) 극한 최적화**: `num_ctx: 2048` 및 `num_thread: 4` 파라미터를 강제 적용하여 불필요한 메모리 스와핑을 방지하고 멀티코어 연산 효율을 극대화했습니다.
...
4. **로컬 LLM 타임아웃 및 자원 튜닝**: 저전력 환경(N150 등)을 고려해 타임아웃을 180초로 여유 있게 설정했으며, `num_ctx: 2048`과 `num_thread: 4`를 기본 옵션으로 주입해 CPU 효율을 높이고 안정성을 확보했습니다. 하드웨어 사양에 따라 가벼운 모델(`llama3.2:1b` 등)로 다운그레이드하거나 옵션을 조절할 수 있습니다.
