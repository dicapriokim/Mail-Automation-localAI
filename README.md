...
- **🧹 지능형 클린업 (Auto-Cleanup)**: 최신 30일 이내의 범용 광고성 프로모션, 불필요한 알림 등을 싹쓸이하여 휴지통으로 이동시켜 편지함을 쾌적하게 유지합니다.
- **⚡ 하이브리드 배치 요약 엔진 및 병렬화 (V5 Optimization)**: 
  - **수집 병렬화 (Parallel I/O)**: Gmail API와 Naver IMAP 수집 구간을 직렬에서 병렬(`Promise.all`)로 개편하여 전체 수집 시간을 절반으로 단축했습니다.
  - **Shift-Left 필터링**: `isStaticBypass` 로직을 통해 광고성 메일을 LLM 호출 없이 즉시 판별하여 비용을 절감합니다.
  - **JSON 환각(Hallucination) 방어**: 저전력 CPU(N150 등) 환경에서 LLM의 배열 포맷 붕괴 현상을 방어하기 위해 `CHUNK_SIZE=1` 단위의 순차 처리로 개편하고 포맷 자동 교정을 적용했습니다.
  - **System Prompt 완전 격리 및 줄바꿈 치환**: 긴 이메일 텍스트에 압도되어 환각이 발생하는 것을 막기 위해 System Prompt를 분리 강제하고, 구글 독스 렌더링 붕괴를 방지하고자 요약문 내 줄바꿈(`\n`)을 공백으로 자동 치환합니다.
  - **Ollama CPU(N150) 극한 최적화**: `num_ctx: 1024` 및 텍스트 600자 제한, `num_thread: 4` 파라미터를 강제 적용하여 불필요한 메모리 스와핑을 방지하고 멀티코어 연산 효율을 극대화하여 처리 속도를 반토막 냈습니다.
  - **Graceful Fallback**: 완전한 포맷 이탈 발생 시 전체 시스템을 멈추지 않고, 해당 메일만 `[AI 요약 지연]` 상태로 격리하여 안전하게 다음 요약을 진행합니다.
...
- **💬 텔레그램 실시간 알림**: 작업 완료 시 설정된 텔레그램 봇을 통해 소요 시간, 메일 건수, 사용된 모델명, 작성된 구글 독스 보고서 링크를 전송받습니다.
...
- **Main Engine**: Node.js (v18+) / Ollama qwen2.5:3b (로컬 LLM - CPU 권장)
...
4. **로컬 LLM 타임아웃 및 자원 튜닝**: 저전력 환경(N150 등)을 고려해 타임아웃을 180초로 여유 있게 설정했으며, 극한의 속도 향상을 위해 본문 길이를 600자로 제한, `num_ctx: 1024`와 `num_thread: 4`를 기본 옵션으로 주입해 CPU 효율을 높이고 안정성을 확보했습니다. 하드웨어 사양에 따라 가벼운 모델(`llama3.2:1b` 등)로 다운그레이드하거나 옵션을 조절할 수 있습니다.
